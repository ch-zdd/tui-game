...
int random_battler_position(battle_type_t type);
...
void round_start(int round_index)
{
    battle_report("$2{Round %d start!}", round_index);
    sort_action();
...
int battle()
{
    ...
    while(round_no<8){
        ...
        for(attacker_idx = 0; attacker_idx < gbattlers_num; attacker_idx++){
            ...
            if(defender_idx < 0) {
                log_info("no defender for attacker %d", attacker_idx);
                round_no = 8;
                break;
            }
            attack(&gbattlers[attacker_idx], &gbattlers[defender_idx]);
            revert_battler_info_screen();
            //if(battle_is_over(&gbattlers[attacker_idx], &gbattlers[defender_idx])) return TG_OK;
        }

        if(round_no == 8){
            battle_report("$2(The battle ends!)");
            break;
        }
...
int select_defender(int attacker_idx)
{
    ...
    if(gbattlers[attacker_idx].type == BATTLE_TYPE_ENEMY){
        defender_position = random_battler_position(BATTLE_TYPE_SELF); //需要判断角色是否已经死亡
        log_debug("select enemy position %d", defender_position);
        for(int i = 0; i < gbattlers_num; i++){
            log_debug("type %d, pos %d", gbattlers[i].type, gbattlers[i].position);
...
int random_battler_position(battle_type_t type)
{
    int pos = rand()%MAX_ENEMY_BATTLER_NUM;
    int max_battle_num = type == BATTLE_TYPE_ENEMY ? MAX_ENEMY_BATTLER_NUM : MAX_SELF_BATTLER_NUM;
    int count = 0;

    while(count < max_battle_num){
        if(get_battler_position_state(type, pos) == POSITION_STATE_ALIVE){
            return pos;
        }else{
            pos = (pos+1) % max_battle_num;
        }
...
int attack(battler_t* attacker, battler_t* defender)
{
    ...
    battle_report("%s$4{[%s] start action}", header, attacker->name);
    ...
    //连击
    if(TG_RANDOM_SUCCESS(combo_chance)){
...
int calc_damage(battler_t* attacker, battler_t* defender, float damage_extra_ratio)
{
    ...
    if(TG_RANDOM_SUCCESS(crit_chance)){
...
void load_innate_attr_buff(battler_t* battler, int skill_value, tk_attr_state_t attr_state, int duration)
{
    ...
    for(j = 1; j<ATTR_TYPE_ALL; j = j<<1){
        if(skill_value & j){
            //tg_print("[%s] load innate attr buff value %x, j = %x", battler->name, skill_value, j);
            switch(j){
